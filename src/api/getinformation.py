...
from src.api.ChatGPTClient import ChatGPTClient
...
def getYinAndYang():
    try:
        ...
        if row:
            ...
            askInfo = ChatGPTClient()

            try:
                response_data = askInfo.query(query)
...
@getinformation_api.route('/getTypeData', methods=['POST'])
def getTypeData():
    try:
        data = request.json
        logger.info(" /getinformation/getTypeData Received request data: %s", data)
        ...
        logger.debug(f"Parsed parameters - gender: {gender}, birth: {birth}, key: {key}")

        finalkey = which_type_belongs(key)
        if not finalkey:
            logger.warning(f"Invalid key provided: {key}")
            return jsonify({"error": "Invalid key"}), 400
        else:
            logger.info(f"Key {key} mapped to fortune type: {finalkey}")

        chat_client = ChatGPTClient()
        query_content = f"请分析八字{birth}，{gender}，在{finalkey}上面的运势分析，用第三人称日主表达出来，请写出来，50字以内，请用英文写。"

        logger.info("Querying ChatGPT API with content: %s", query_content)
        result = chat_client.query(query_content)

        logger.info("八字排盘result: %s", result)

        return jsonify({"data": result,'key':key}), 200


    except ValueError as ve:
        logger.exception("ValueError occurred while processing request")
        return jsonify({"error": "Invalid request data", "details": str(ve)}), 400
    except Exception as e:
...
def which_type_belongs(key: str) -> Optional[str]:
    match key:
        case "2":
            return "Wealth Forecast"
        case "3":
            return "Career Fortune"
        case "4":
            return "Windfall Fortune"
        case "5":
            return "Nobleman Fortune"
        case "6":
            return "Love Fortune"
        case "7":
            return "Love Attraction Luck"
        case _:
            return None  # 如果 key 不匹配，返回 None
